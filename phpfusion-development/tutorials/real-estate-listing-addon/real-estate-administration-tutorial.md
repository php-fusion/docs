...
Finally, we can copy all the functions above and paste everything into a file or below the controller file after line 15.
...
// Private function - get the latest navigation status and display the nav status.
// Since the data model function ran before hand, the set_property_nav_status should be true if rows are more.
function get_property_nav_status($limit = 20) {
    // Let's begin
    if (set_property_nav_status() === TRUE) {
